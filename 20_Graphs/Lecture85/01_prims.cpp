...
ll prims(int src, int n){
    ...
    while(total_count < n  &&  !pq.empty()){
        ...
        for(auto neighbour: gr[curr.second]){
            //here first element of graph vector is node and 2nd is weight
            if(!vis.count(neighbour.first) && mp[neighbour.first] > neighbour.second){//we have found the better weight
